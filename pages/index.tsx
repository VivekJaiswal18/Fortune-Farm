...
const Home: NextPage = () => {
  ...
  if (!address) {
    return (
      <Container maxW={"1200px"} marginTop={20}>
        <Flex direction={"column"} h={"100vh"} justifyContent={"center"} alignItems={"center"}>
          <Heading mt={"40px"} my={"40px"}>Welcome to Fortune Farm</Heading>
          <ConnectWallet />
        </Flex>
      </Container>
    );
  ...
  return (
    <Box pt="80px"> {/* This padding ensures content is not hidden behind the navbar */}
      <Container maxW={"1200px"}>
        <SimpleGrid columns={2} spacing={10}>
          <Card p={5}>
            ...
            <SimpleGrid columns={2} spacing={10}>
              ...
              <Box>
                <Text fontSize={"small"} fontWeight={"bold"}>$FARM COIN Balance:</Text>
                {rewardBalance && (
